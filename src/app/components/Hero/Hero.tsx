    ...
    <HeroFrame>
      <div className="hero-image-wrap">
        <motion.div className="hero-image-mask" initial={reduceMotion ? false : { clipPath: "inset(0 0 0 100%)" }} animate={{ clipPath: "inset(0 0 0 0%)" }} transition={{ duration: 1.1, ease: [0.22, 1, 0.36, 1] }}>
          <Image src={image} alt="ARV architectural and construction work" fill priority sizes="(max-width: 900px) 100vw, 62vw" className="hero-image" />
        </motion.div>
      ...
      <div className="hero-copy site-shell">
        ...
        <motion.div className="hero-actions" initial={reduceMotion ? false : { opacity: 0, y: 20 }} animate={enter} transition={{ duration: 0.7, delay: 0.55 }}>
          <Link href="/projects" className="button button-primary">View projects <span>↗</span></Link>
          <Link href="/request-quote" className="button button-ghost">Start a project <span>→</span></Link>
        </motion.div>
      </div>
